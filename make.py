#!/usr/bin/env python3
"""
FLUXUS build script wrapping conda-build, and exposing matrix
...
FLUXUS_PATH_ENV = "FLUXUS_PATH"
FLUXUS_PATH_URI_ENV = "FLUXUS_PATH_URI"
FLUXUS_BUILD_PKG_VERSION_ENV = "FLUXUS_BUILD_{project}_VERSION"
FLUXUS_DEPENDENCY_VERSION_ENV_PREFIX = "FLUXUS_V_"
PATTERN_ENV_VERSION_REFERENCE = re.compile(
    r"{env:(" + FLUXUS_DEPENDENCY_VERSION_ENV_PREFIX + r"\w+):?}"
)
CONDA_BUILD_PATH_ENV = "CONDA_BLD_PATH"
FLUXUS_NO_BINARY_ENV = "FLUXUS_NO_BINARY"
...
class Builder(metaclass=ABCMeta):
    def __init__(self, project: str, dependency_type: str) -> None:
        ...
        os.environ[FLUXUS_PATH_URI_ENV] = (
        ...
        os.environ[FLUXUS_BUILD_PKG_VERSION_ENV.format(project=project.upper())] = (
            package_version
        )
    ...
    def make_build_path(self) -> str:
        ...
        return os.path.abspath(
            os.path.join(os.environ[FLUXUS_PATH_ENV], self.build_path_suffix)
        )
    ...
    def expose_package_dependencies(self) -> Mapping[str, str]:
        ...
        export_environment_variable(
            name=FLUXUS_NO_BINARY_ENV, value=",".join(build_no_binaries)
        )
...
class CondaBuilder(Builder):
    def __init__(self, project: str, dependency_type: str):
        ...
        if " " in self.projects_root_path:
            warnings.warn(
                ...
                f"environment variable {FLUXUS_PATH_ENV} ahead of running make.py."
            )
    ...
    def build(self, exposed_package_dependencies: Mapping[str, str]) -> None:
        """
        Build a FLUXUS project using conda-build.
        ...
        recipe_path = os.path.abspath(
            os.path.join(os.environ[FLUXUS_PATH_ENV], self.project, "condabuild")
        )
...
class ToxBuilder(Builder):
    ...
    def build(self, exposed_package_dependencies: Mapping[str, str]) -> None:
        """
        Build a FLUXUS project using tox.
    ...
    def _get_tox_ini_path(self) -> str:
        ...
        tox_ini_dir = os.path.join(os.environ[FLUXUS_PATH_ENV], self.project)
...
def get_projects_root_path() -> str:
    if (FLUXUS_PATH_ENV in os.environ) and os.environ[FLUXUS_PATH_ENV]:
        fluxus_path = os.environ[FLUXUS_PATH_ENV]
    else:
        fluxus_path = os.path.abspath(os.path.join(SCRIPT_DIR, os.pardir))
        export_environment_variable(name=FLUXUS_PATH_ENV, value=fluxus_path)
